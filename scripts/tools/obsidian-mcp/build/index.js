...
async function robustSearch(query) {
    ...
    for (const note of allNotes) {
        ...
        if (normalizedNoteName.includes(normalizedQuery))
            score += 10;
        // Peso 5: Metadados/Tags
        if (normalize(metadata).includes(normalizedQuery))
            score += 5;
    ...
    const formatted = searchResults.slice(0, 10).map(res => `### [[${res.note.replace(".md", "")}]] (Score: ${res.score})\n> ${res.excerpt}\n`);
